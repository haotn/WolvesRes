...
public class BanDAO implements WolvesResDAO<ModelBan, String> {
    ...
    public ModelBan selectById(String ID) {
        List<ModelBan> list = new ArrayList<>();
        list = selectBySQL("select * from BAN where MaBan like ? or TenBan like ?", ID);
        return list.get(0);
    }
    ...
    public List<ModelBan> selectBySQL(String sql, Object... thamSo) {
        ...
        try {
            ...
            try {
                ...
                while (result.next()) {
                    ModelBan entity = new ModelBan(result.getString(1), result.getString(2), result.getBoolean(3), result.getString(4));
                    list.add(entity);
                }
            } finally {
