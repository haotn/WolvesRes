...
public class SanPhamDAO implements WolvesResDAO<ModelSanPham, String> {
    ...
    public List<ModelSanPham> selectBySQL(String sql, Object... thamSo) {
        ...
        try {
            ...
            try {
                ...
                while (result.next()) {
                    ModelSanPham sanPham = new ModelSanPham(result.getString(1), result.getString(2), result.getFloat(3), result.getString(4), result.getString(5), result.getInt(6), result.getBoolean(7));
                    list.add(sanPham);
                }
            } finally {
    ...
    public void insert(ModelSanPham sanPhamMoi) {
        try {
            ...
            XJdbc.update(sql, sanPhamMoi.getMaSP(), sanPhamMoi.getTenSP(), sanPhamMoi.getGiaBan(), sanPhamMoi.getMaDanhMuc(), sanPhamMoi.getPathAnh(), sanPhamMoi.getMaDVT(), sanPhamMoi.isTrangThai());
    ...
    public void update(ModelSanPham sanPhamMoi, String ID) {
        try {
            ...
            XJdbc.update(sql, sanPhamMoi.getTenSP(), sanPhamMoi.getGiaBan(), sanPhamMoi.getMaDanhMuc(), sanPhamMoi.getPathAnh(), sanPhamMoi.getMaDVT(), sanPhamMoi.isTrangThai(), sanPhamMoi.getMaSP());
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    }
    ...
    public ModelSanPham checkForeignKho(String ID) {
        ...
        try {
            ...
            if (result.next()) {
                entity = new ModelSanPham(result.getString(1), result.getString(2), result.getFloat(3), result.getString(4), result.getString(5), result.getInt(6), result.getBoolean(7));
            }
        } catch (SQLException ex) {
