...
public class FormVoucher extends javax.swing.JPanel {
    ...
    private List<ModelVouCher> list = new ArrayList<ModelVouCher>();
    private List<ModelVouCher> whiteList = new ArrayList();
    ...
    private EventAction<ModelVouCher> eventAction = new EventAction<ModelVouCher>() {
        ...
        public void update(ModelVouCher entity) {
            ...
            if (!editForm.isDidpose()) {
                updateVoucher(editForm.getVoucher(), entity.getMaVoucher());
            }
        }
        ...
        public void delete(ModelVouCher entity) {
            if (dao.checkForeignKey(entity) == null) {
                if (ROptionDialog.showConfirm(frame,"Xác nhận", "Bạn có chắc muốn xóa voucher không?", ROptionDialog.WARNING, Color.yellow, Color.black)) {
...
//                    tblVoucher.setRowSelectionInterval(index, index);
                ...
                ROptionDialog.showAlert(frame,"Thông báo", "Voucher đã được sử dụng, không thể xóa!", ROptionDialog.WARNING, Color.red, Color.black);
            }
        }
    };
    ...
    public void init() {
        ...
        Icon iconXemDSDen = IconFontSwing.buildIcon(GoogleMaterialDesignIcons.FEATURED_PLAY_LIST, 32, new Color(0, 199, 135));
        Icon iconAddDSDen = IconFontSwing.buildIcon(GoogleMaterialDesignIcons.REMOVE_CIRCLE, 32, new Color(255, 51, 51));
    ...
    private void initTable() {
        ...
        if (!Auth.isBoss() && !Auth.isManager()) {
            ...
            model = new DefaultTableModel(new Object[][]{}, new Object[]{"Mã Voucher", "Giảm giá", "Số lượng", "Thao tác"});
        }
        tblVoucher.setModel(model);
        tblVoucher.setColumnAction(3);
    }

    private void insertVoucher(ModelVouCher vc) {
        dao.insert(vc);
        list.add(vc);
        fillToTable();
    }

    private void updateVoucher(ModelVouCher vc, String id) {
        dao.update(vc, id);
        for (int i = 0; i < list.size(); i++) {
            if (id.equals(list.get(i).getMaVoucher())) {
                list.set(i, vc);
            }
        }
        fillToTable();
    ...
    private void showDetail(ModelVouCher vc) {
        if (vc != null) {
            if (vc.getPathQR() != null) {
                ImageIcon pic = new ImageIcon(XImage.readImageQRCode(vc.getPathQR()).getImage().getScaledInstance(250, 250, Image.SCALE_SMOOTH));
                lblQR.setIcon(pic);
    ...
    private void deleteVoucher(ModelVouCher vc) {
        dao.delete(vc.getMaVoucher());
        list.remove(vc);
        fillToTable();
    }

    private void updateToList(ModelVouCher vc) {
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).getMaVoucher().equals(vc.getMaVoucher())) {
                list.set(i, vc);
            }
        }
    }

    private void findVoucher(String keyword) {
        List<ModelVouCher> listFind = new ArrayList<>();
        listFind.clear();
        for (int i = 0; i < whiteList.size(); i++) {
            if (keyword.trim().length() != 0) {
                if (whiteList.get(i).getMaVoucher().contains(keyword) || whiteList.get(i).getNgayBatDau().contains(keyword)
                        || whiteList.get(i).getNgayKetThuc().contains(keyword)) {
                    listFind.add(whiteList.get(i));
                    model.setRowCount(0);
                    for (ModelVouCher vc : listFind) {
                        tblVoucher.addRow(vc.toRowTable(eventAction));
                    }
                }
            } else {
        ...
        if (bls.isChangeData()) {
            for (int i = 0; i < list.size(); i++) {
                for (int j = 0; j < bls.getListReturn().size(); j++) {
                    if (list.get(i).getMaVoucher().equals(bls.getListReturn().get(j).getMaVoucher())) {
                        updateToList(bls.getListReturn().get(j));
                    }
                }
            }
            fillToTable();
    ...
    private void bntThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntThemActionPerformed
        ...
        editForm.setVisible(true);
        if (!editForm.isDidpose()) {
            insertVoucher(editForm.getVoucher());
    ...
    private void txtTimKiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTimKiemKeyReleased
        findVoucher(txtTimKiem.getText().trim());
    ...
    private void btnVoHieuHoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnVoHieuHoaActionPerformed
        if (index >= 0) {
            if (ROptionDialog.showConfirm(frame,"Xác nhận", "Bạn có chắc muốn vô hiệu hóa Voucher không?", ROptionDialog.WARNING, Color.yellow, Color.black)) {
                ModelVouCher vc = getVoucherFromRowTable(index);
                vc.setTrangThai(false);
                updateVoucher(vc, vc.getMaVoucher());
            }
        }
