...
public class BlackListVoucher extends javax.swing.JDialog {
    ...
    private void updateVoucher(ModelVouCher vc) {
        changeData = true;
        vc.setTrangThai(true);
        dao.update(vc, vc.getMaVoucher());
        for (int i = 0; i < formParent.getList().size(); i++) {
            if (formParent.getList().get(i).getMaVoucher().equals(vc.getMaVoucher())) {
                formParent.getList().set(i, vc);
            }
        }
        listReturn.add(vc);
        fillToTable();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(BlackListVoucher.class
