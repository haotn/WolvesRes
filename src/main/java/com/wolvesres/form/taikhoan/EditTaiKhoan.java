...
//sửa setFrom và getForm trong Edittaikhoan
    ...
    public boolean valideForm() {
        ...
        if (Tennv.length() == 0 || Matkhau.length() == 0 || NhaplaiMK.length() == 0) {
            ...
            if (!NhaplaiMK.equalsIgnoreCase(Matkhau)) {
                ROptionDialog.showAlert(frame, "Lỗi", "Nhập lại mật khẩu chưa chính xác", ROptionDialog.WARNING, Color.red, Color.black);
                return false;
            }
            if (!validatePass(Matkhau)) {
                ROptionDialog.showAlert(frame, "Lỗi", "Vui lòng nhập từ 6 đến 16 ký tự", ROptionDialog.WARNING, Color.red, Color.black);
                return false;
            }
        }
        setTaiKhoan(getForm());
    ...
    private void btnHuyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnHuyActionPerformed
        if (ROptionDialog.showConfirm(frame, "Xác nhận", "Bạn có chắc muốn hủy không?", ROptionDialog.WARNING, Color.yellow, Color.black)) {
            dispose();
        }
