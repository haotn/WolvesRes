...
public class FormSanPham extends javax.swing.JPanel {
    ...
    EventAction<ModelSanPham> eventAction = new EventAction<ModelSanPham>() {
        public void delete(ModelSanPham sanpham) {
            if (daoSanPham.checkForeignKho(sanpham.getMaSP()) == null && daoSanPham.checkForeignChiTietLS(sanpham.getMaSP()) == null && daoSanPham.checkForeignHoaDonCT(sanpham.getMaSP()) == null) {
                if (ROptionDialog.showConfirm(frame, "Xác nhận", "Xác nhận xóa sản phẩm?", ROptionDialog.WARNING, Color.yellow, Color.black)) {
                ...
                if (ROptionDialog.showConfirm(frame, "Thông báo xác nhận", "Không thể xóa, xác nhận ngưng kinh doanh sản phẩm?", ROptionDialog.PRIORITY_HIGHT, Color.red, Color.black)) {
        ...
        public void update(ModelSanPham sanpham) {
            ...
            if (editForm.getIsDispose() == false) {
                ...
                fillToTable(danhMuc.getMaDanhMuc());
                ROptionDialog.showAlert(frame, "Thông báo", "Cập nhật thành công!", ROptionDialog.NOTIFICATIONS, new Color(0, 199, 135), Color.black);
...
/////////////////////////////////////////////////////
    ...
    public void init() {
        ...
        Icon iconXemDSDen = IconFontSwing.buildIcon(GoogleMaterialDesignIcons.REMOVE_CIRCLE, 32, new Color(0, 199, 135));
...
/////////////////////////////////////////////////////////
    ...
    private void initTable() {
        ...
        if (!Auth.isBoss() && !Auth.isManager()) {
            model = new DefaultTableModel(new Object[][]{}, new Object[]{"Tên sản phẩm", "Danh mục", "Giá bán", "Loại"});
        } else {
            model = new DefaultTableModel(new Object[][]{}, new Object[]{"Tên sản phẩm", "Danh mục", "Giá bán", "Loại", "Thao tác"});
        }
    ...
    private void btnDuaVaoDSDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDuaVaoDSDActionPerformed
        if (index >= 0) {
            if (ROptionDialog.showConfirm(frame, "Xác nhận", "Bạn có chắc chắn muốn đưa vào danh sách đen?", ROptionDialog.WARNING, Color.yellow, Color.black)) {
                ModelSanPham entity = whiteList.get(tblSanpham.getSelectedRow());
                addToBlackList(entity);
            }
        } else {
            ROptionDialog.showAlert(frame, "Thông báo", "Vui lòng chọn dữ liệu!", ROptionDialog.WARNING, Color.red, Color.black);
        }
