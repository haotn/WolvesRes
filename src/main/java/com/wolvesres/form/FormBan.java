...
public class FormBan extends javax.swing.JPanel {
    ...
    private void loadToListBanComp() {
        ...
        for (int i = 0; i < listBan.size(); i++) {
            ...
            for (int k = 0; k < Main.listBanOrder.size(); k++) {
                if (ban.getMaBan().equals(Main.listBanOrder.get(k).getMaBan()) || ban.getMaBan().equals(Main.listBanOrder.get(k).getMaBanGop())) {
                    if (ban.getMaBan().equals(Main.listBanOrder.get(k).getMaBan()) || ban.getMaBan().equals(Main.listBanOrder.get(k).getMaBanGop())) {
                        active = true;
                    }
                }
            ...
            banComp.setEventDelete(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    if (banDAO.checkForeignKey(ban).getMaBan() == null) {
                        if (ROptionDialog.showConfirm(frame, "Xác nhận", "Xác nhận xóa bàn?", ROptionDialog.WARNING, Color.RED, Color.black)) {
                            deleteTable(ban);
                            ModelKhuBan khuBan = (ModelKhuBan) cboKhuBan.getSelectedItem();
                            fillPanelBan(khuBan);
                        }
                    } else {
                        ROptionDialog.showAlert(frame, "Thông báo", "Không thể xóa bàn này!", ROptionDialog.NOTIFICATIONS, Color.blue, Color.black);
                    }
                }
            });
    ...
    private void insertTable(ModelBan ban) {
        banDAO.insert(ban);
        listBan.add(ban);
        ModelKhuBan khuBan = (ModelKhuBan) cboKhuBan.getSelectedItem();
        fillPanelBan(khuBan);
    ...
    private void updateTable(ModelBan ban) {
        banDAO.update(ban, ban.getMaBan());
        for (int i = 0; i < listBan.size(); i++) {
            if (listBan.get(i).getMaBan().equals(ban.getMaBan())) {
                listBan.set(i, ban);
            }
        }
        ModelKhuBan khuBan = (ModelKhuBan) cboKhuBan.getSelectedItem();
    ...
    private void deleteTable(ModelBan ban) {
        banDAO.delete(ban.getMaBan());
        listBan.remove(ban);
        ModelKhuBan khuBan = (ModelKhuBan) cboKhuBan.getSelectedItem();
        fillPanelBan(khuBan);
    }
