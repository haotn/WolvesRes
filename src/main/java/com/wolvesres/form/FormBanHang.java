        ...
        if (listOrder.size() > 0) {
            for (int i = 0; i < listOrder.size(); i++) {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        if (!orderComp.isEdit()) {
                            if (orderComp.updateProduct()) {
                                updateOrder(orderComp.getProduct());
                                tinhTongTien();
                            } else {
                                ROptionDialog.showAlert(frame, "Thông báo", "Cập nhật không thành công!", ROptionDialog.NOTIFICATIONS, Color.red, Color.black);
                            }
                        }
                    }
                });
                orderComp.setActionDelete(new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                        if (ROptionDialog.showConfirm(frame, "Xác nhận", "Xác nhận xóa sản phẩm khói danh sách gọi món?", ROptionDialog.WARNING, Color.yellow, Color.black)) {
                            deleteOrder(entity);
                            tinhTongTien();
                        }
                    }
...
//        int xShift = 280;
        ...
        for (int i = 0; i < listOrderComp.size(); i++) {
            ...
            if (entity.getProduct().getSanPham().getPathAnh() != null) {
                entity.getProduct().getSanPham().setIcon(XImage.readImageThucDon(entity.getProduct().getSanPham().getPathAnh()));
            } else {
    ...
    public void setForm() {
        ...
        if (!banOrderGlobal.getMaVoucher().equals("NOVOUCHER")) {
            ...
            // btnRefreshVoucher.setIcon(new ImageIcon(getClass().getResource("/com/wolvesres/icon/refreshVoucher.png")));
            confirm = false;
            isVoucherValid = true;
        }
        tinhTongTien();
     ...
     * Data
     */
    private void updateBanOrder(ModelBanOrder entity) {
    ...
    private void checkMaVoucher(String keyword) {
        for (int i = 0; i < listVouCher.size(); i++) {
            if (keyword.equals(listVouCher.get(i).getMaVoucher()) && !keyword.equals("NOVOUCHER")) {
                if (listVouCher.get(i).isTrangThai()) {
                    if ((listVouCher.get(i).getSoLuong() > 0 && checkForApply) || (listVouCher.get(i).getSoLuong() >= 0 && !checkForApply)) {
    ...
    private void applyVoucher() {
        if (isVoucherValid) {
            ...
            banOrderGlobal.setMaVoucher(voucher.getMaVoucher());
            updateBanOrder(banOrderGlobal);
            ...
            checkMaVoucher(voucher.getMaVoucher());
    ...
    private void updateVoucher(ModelVouCher entity) {
        voucherDAO.update(entity, entity.getMaVoucher());
        for (int i = 0; i < listVouCher.size(); i++) {
    ...
    private float tinhGiamGia() {
        float giamGia = 0;
        if (isVoucherValid) {
            ModelVouCher voucher = getVoucherByMaVouCher(lblMaVoucher.getText().trim());
    ...
    private float tinhTongTien() {
        float tongTien = 0;
        tongTien = tinhTienHang() - tinhGiamGia();
        float tienTraLai = 0;
        try {
            ...
            ROptionDialog.showAlert(frame, "Lỗi định dạng", "Vui lòng nhập giá trị số!", ROptionDialog.REPORT, Color.RED, Color.black);
        }
    ...
    /**
     * Thao tac voi sanPham order
     */
    private void insertOrder(ModelOrder entity) {
        orderDAO.insert(entity);
        listOrder.add(entity);
        fillToPnlOrder();
    }

    private void updateOrder(ModelOrder entity) {
        orderDAO.update(entity, entity.getMaBan());
        for (int i = 0; i < listOrder.size(); i++) {
            if ((entity.getMaBan().equals(listOrder.get(i).getMaBan())) && (entity.getMaSP().equals(listOrder.get(i).getMaSP()))) {
    ...
    private void deleteOrder(ModelOrder entity) {
        orderDAO.deleteProduct(entity.getMaSP(), entity.getMaBan());
    ...
    private void orderSanPham(ModelSanPham sanPham) {
        ModelOrder order = null;
        for (int i = 0; i < listOrder.size(); i++) {
            if ((banOrderGlobal.getMaBan().equals(listOrder.get(i).getMaBan())) && (sanPham.getMaSP().equals(listOrder.get(i).getMaSP()))) {
                order = listOrder.get(i);
                break;
            }
        }
            ...
            if (listSPInKho.size() > 1) {
                for (int i = 0; i < listSPInKho.size(); i++) {
                    for (int j = 0; j < listSPInKho.size(); j++) {
                        if (XDate.toDate(listSPInKho.get(i).getHanSuDung(), "dd-MM-yyyy").before(XDate.toDate(listSPInKho.get(j).getHanSuDung(), "dd-MM-yyyy"))) {
                            kho = new ModelKho();
    ...
    private void updateKho(ModelKho entity) {
        khoDAO.update(entity, entity.getId());
        for (int i = 0; i < listKho.size(); i++) {
            if (entity.getMaSP().equals(listKho.get(i).getMaSP()) && entity.getHanSuDung().equals(listKho.get(i).getHanSuDung())) {
    ...
    private void thanhToan() {
        ...
        if (acceptPtint) {
            ...
            float tienKhachDua = Float.parseFloat(txtTienKhachDua.getText().trim());
            BillPrint print = new BillPrint(hoaDon, time, this.giamGia, tienKhachDua, this.tienTraLai, listHoaDonChiTiets, listSanPhams, listVouCher, getRootPane());
            print.print();
        }
    }
    ...
    private boolean checkTienKhachDua() {
        if (txtTienKhachDua.getText().trim().length() == 0) {
            ROptionDialog.showAlert(frame, "Thông báo", "Vui lòng nhập tiên khách đưa!", ROptionDialog.WARNING, Color.red, Color.black);
            return false;
        } else if (tienTraLai < 0) {
            ROptionDialog.showAlert(frame, "Thông báo", "Không cho nợ nhé!", ROptionDialog.WARNING, Color.red, Color.black);
            return false;
        }
        return true;
    }
    ...
    private void find(String keyword) {
        ...
        if (keyword.length() != 0) {
            for (int i = 0; i < listSanPhamComp.size(); i++) {
                if (listSanPhamComp.get(i).getProduct().getTenSP().toUpperCase().contains(keyword) || listSanPhamComp.get(i).getProduct().getTenSP().toUpperCase().contains(keyword)) {
    ...
    private void btnHuyHoaDonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnHuyHoaDonActionPerformed
        if (ROptionDialog.showConfirm(frame, "Xác nhận", "Xác nhận hủy hóa đon bán hàng?", ROptionDialog.WARNING, Color.YELLOW, Color.black)) {
            huyHoaDon();
        }
    ...
    private void btnThanhToanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThanhToanActionPerformed
        if (listOrder.size() > 0) {
            if (checkTienKhachDua()) {
                if (ROptionDialog.showConfirm(frame, "Tùy chọn", "Bạn có muốn in hóa đơn không?", ROptionDialog.NOTIFICATIONS_ACTIVE, Color.yellow, Color.black)) {
                    acceptPtint = true;
                }
                thanhToan();
                huyHoaDon();
            ...
            ROptionDialog.showAlert(frame, "Thông báo", "Vui lòng gọi món trước khi thanh toán", ROptionDialog.WARNING, Color.red, Color.black);
        }
    ...
    private void txtTienKhachDuaKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTienKhachDuaKeyPressed
        if (evt.getKeyChar() == KeyEvent.VK_ENTER) {
            ...
            if (this.tienTraLai < 0) {
                ROptionDialog.showAlert(frame, "Thông báo", "Tiền khách đưa phải lớn hơn hoặc bằng tổng tiên!", ROptionDialog.WARNING, Color.red, Color.black);
            }
