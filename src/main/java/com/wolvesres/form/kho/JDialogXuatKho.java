...
////////////////////////
    ...
    private void initTableKho() {
        DefaultTableModel modelSanPham = new DefaultTableModel(new Object[][]{}, new Object[]{"Sản phẩm", "SL", "Giá", "HSD"});
...
///////
    ...
    public boolean checkSL(int select) {
        ...
        try {
            ...
            if (sl <= 0) {
                ...
                ROptionDialog.showAlert(frame, "Lỗi", "Số lượng xuất kho tối thiểu là 1!", ROptionDialog.WARNING, Color.red, Color.black);
                return false;
            }
            slkho = listNhapKho.get(select).getSoLuong();
            if (sl > slkho) {
                ROptionDialog.showAlert(frame, "Lỗi", "Số lượng xuất vượt quá số lượng đang có Trong kho :" + (sl - slkho), ROptionDialog.WARNING, Color.red, Color.black);
                sl = listNhapKho.get(select).getSoLuong();
                txtSoLuong.setText(sl + "");
                return false;
            }
    ...
    public boolean checkGia(int select) {
        ...
        try {
            ...
            if (gia <= 0) {
                ...
                txtGia.setText(gia + "");
                ROptionDialog.showAlert(frame, "Lỗi", "Giá phải lớn hơn không!", ROptionDialog.WARNING, Color.red, Color.black);
                return false;
            }
            ...
            if (gia < gt) {
                gia = listNhapKho.get(select).getGia();
                txtGia.setText(gia + "");
                ROptionDialog.showAlert(frame, "Lỗi", "Giá nhập quá nhỏ so với giá nhập ban đầu!", ROptionDialog.WARNING, Color.red, Color.black);
                return false;
            }
        } catch (Exception e) {
            gia = listNhapKho.get(select).getGia();
    ...
    public void XuatKho() {
        if (ROptionDialog.showConfirm(frame, "Xác nhận", "Xác nhận xuất kho?", ROptionDialog.WARNING, Color.yellow, Color.black)) {
            String NV = Auth.user.getMaNV();
            if (listNhapKho.size() < 0) {
                ROptionDialog.showAlert(frame,"Thông báo", "Vui lòng chọn Sản Phẩm để nhập", ROptionDialog.PRIORITY_HIGHT, Color.red, Color.black);
            } else {
                //Lịch sử
...
//    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JDialogXuatKho.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
