...
public class JDialogNhapKho extends javax.swing.JDialog {
    ...
    private void init() {
        ...
        Date today = new Date();
        dateChooser.setSelectedDate(XDate.addDays(XDate.toDate(XDate.toString(today, "dd-MM-yyyy"), "dd-MM-yyyy"), 365));
    ...
    private void initTableKho() {
        DefaultTableModel modelKho = new DefaultTableModel(new Object[][]{}, new Object[]{"Sản phẩm", "Số lượng", "Giá", "Hạn SD", "Thao tac"});
    ...
    private void initTableSP() {
        DefaultTableModel modelSanPham = new DefaultTableModel(new Object[][]{}, new Object[]{"Sản phẩm", "Danh mục", "Giá"});
    ...
    public boolean checkSL(int select) {
        ...
        try {
            ...
            if (sl <= 0) {
                ...
                ROptionDialog.showAlert(frame, "Lỗi", "Số lượng nhập ít nhất là 1!", ROptionDialog.WARNING, Color.red, Color.black);
                return false;
            }
        } catch (Exception e) {
            sl = listNhapKho.get(select).getSoLuong();
            txtSoLuong.setText(sl + "");
            ROptionDialog.showAlert(frame, "Lỗi", "Số lượng phải là số!", ROptionDialog.WARNING, Color.red, Color.black);
            return false;
        }
        return true;
    }
    ...
    public boolean checkGia(int select) {
        ...
        try {
            ...
            if (gia > gt) {
                gia = listNhapKho.get(select).getGia();
                txtGia.setText(gia + "");
                ROptionDialog.showAlert(frame, "Lỗi", "Giá nhập quá lớn so với giá hiện tại!", ROptionDialog.WARNING, Color.red, Color.black);
                return false;
            }
        } catch (Exception e) {
            gia = listNhapKho.get(select).getGia();
...
//        }
        if (!day.after(minDay)) {
            ROptionDialog.showAlert(frame, "Lỗi", "Hạn sử dụng ít nhất!", ROptionDialog.WARNING, Color.red, Color.black);
            txtNgayHetHan.setText(listNhapKho.get(select).getHanSD());
            return false;
        }
        return true;
    ...
    public void NhapKho() {
        if (ROptionDialog.showConfirm(frame, "Xác nhận", "Xác nhận nhập kho?", ROptionDialog.WARNING, Color.yellow, Color.black)) {
            String NV = Auth.user.getMaNV();
            if (listNhapKho.size() < 0) {
                ROptionDialog.showAlert(frame, "Lỗi", "Vui lòng chọn Sản Phẩm để nhập", ROptionDialog.WARNING, Color.red, Color.black);
            } else {
                //Lịch sử
                ...
                mdLS.setNhapKho(true);
                mdLS.setTongTien(sum);
                mdLS.setNguoiNhap(NV);
                lsDAO.insert(mdLS);
                listLS.clear();
                ...
                for (int i = 0; i < listNhapKho.size(); i++) {
                    ...
                    mdkho.setIdls(idLS);
                    mdkho.setMaSP(MSP);
                    mdkho.setSoLuong(soLuong);
                    mdkho.setHanSuDung(date);
                    ...
                    mdctls.setIdLS(idLS);
                    mdctls.setMaSP(MSP);
                    mdctls.setSoLuong(soLuong);
                    mdctls.setDonGia(Gia);
                    ctDAO.insert(mdctls);
                }
                ROptionDialog.showAlert(frame, "Thông báo", "Nhập Kho Thành Công!", ROptionDialog.NOTIFICATIONS_ACTIVE, new Color(0, 199, 135), Color.black);
...
//            System.out.println(editSP.getSanPham().getMaSP());
            ...
            fillToTableSanPham();
            ROptionDialog.showAlert(frame, "Thông báo", "Thêm sản phẩm thành công!", ROptionDialog.NOTIFICATIONS_ACTIVE, new Color(0, 199, 135), Color.black);
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JDialogNhapKho.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
